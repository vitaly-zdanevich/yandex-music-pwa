...
The service worker keeps the application shell available, while complete audio blobs and artwork are stored in IndexedDB. Preferences lets you keep 0–50 upcoming recommendations offline, defaulting to 10; the chosen limit is stored locally on that device. By default, the rolling cache evicts tracks that fall behind while online and does not prune while offline playback is active. The unchecked-by-default **Do not remove offline tracks** preference changes automatic caching to add-only; manual per-track removal and Remove all still work. The Offline screen reports the exact bytes stored by this app and lets you play or delete individual downloads or clear all of them.
