...
export class CacheCoordinator {
  ...
  private async cacheTrack(track: Track, signal: AbortSignal): Promise<void> {
    ...
    try {
      audioResponse = await fetch(source.url, { signal, cache: 'no-store' });
      if ((!audioResponse.ok || audioResponse.status === 206) && source.proxyUrl && source.proxyUrl !== source.url) {
        audioResponse = await fetch(source.proxyUrl, { signal, cache: 'no-store' });
      }
    } catch (error) {
    ...
    if (!signal.aborted) await this.store.put(track, audio, artwork);
  }
}
